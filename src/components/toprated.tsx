...
/** сопоставляем заголовки карточек id отелей (из вашей базы) */
const HEADING_TO_ID: Record<string, string> = {
  "The Spectator Hotel": "sg-fullerton",
  "Marseilles Beachfront Resort": "dubai-ocean",
  "Waldorf Astoria Los Cabos": "dubai-city",
  "Deco Boutique Hotel": "sg-emma",
};
  ...
  const topRated = [
    {
      ...
      heading: "The Spectator Hotel",
      cost: "2024",
      period: "Night",
      address: "26 State Street, Charleston, USA",
      beds: "2 Luxury Beds",
      sleeps: "4 Guests",
      size: "760 Sq Ft",
    },
    {
      ...
      heading: "Marseilles Beachfront Resort",
      cost: "5024",
      period: "Day and Night",
      address: "1741 Collins Ave, Miami Beach, USA",
      beds: "12 Deluxe Beds",
      sleeps: "24 Guests",
      size: "3,190 Sq Ft",
    },
    {
      ...
      heading: "Waldorf Astoria Los Cabos",
      cost: "2800",
      period: "Day and Night",
      address: "Camino del Mar, Cabo San Lucas, Mexico",
      beds: "5 Oceanview Beds",
      sleeps: "10 Guests",
      size: "2,450 Sq Ft",
    },
    {
      image: "/TopRated/Deco-Boutique-Hotel.svg",
      heading: "Deco Boutique Hotel",
      cost: "1450",
      period: "Day",
      address: "4120 NE 23rd Ave, Fort Lauderdale, USA",
      beds: "2 Boutique Beds",
      sleeps: "4 Guests",
      size: "980 Sq Ft",
    },
  ];
...
function Card({ ItmeMenu }: any) {
  ...
  return (
    <article className="w-full h-auto gap-y-4 lg:gap-y-5 flex flex-col items-start">
      {/* кликом по изображению на моб/таб — показать/скрыть панель */}
      <div
        role="button"
      ...
      >
        ...
        {/* панель с иконками — state на моб, hover на md+ */}
        <div
          id={panelId}
          className={[
      ...
      <div className="w-full h-auto gap-y-1.5 lg:gap-y-2.5 flex flex-col items-start">
        {/* заголовок — Link на страницу отеля */}
        <Link
          href={detailsHref}
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          ...
          {/* НОВОЕ: Select rooms — модалка со счётчиками и типами номеров */}
          <SelectRooms
            hotelId={hotelId}
...
function SelectRooms({
  ...
  const [open, setOpen] = useState(false);
  const [roomIdx, setRoomIdx] = useState(1); // default Deluxe
        ...
        >
          ...
          >
            ...
            <div className="mt-4 flex flex-col gap-2">
              <button
                className="h-10 rounded-md bg-[#1E2352] text-white font-semibold hover:opacity-90"
              ...
              {savedMsg && (
                <div className="text-center text-sm text-emerald-700">{savedMsg}</div>
              )}
            </div>
          </div>
        </div>
...
/** Иконка с подписью — клик на моб/таб, hover на md+ */
function TooltipIcon({ icon, text }: { icon: string; text: string }) {
  const [show, setShow] = useState(false);

  const toggle = (e: React.MouseEvent | React.KeyboardEvent) => {
    e.stopPropagation(); // не закрываем панель карточки
    setShow((v) => !v);
  };
  const onKey = (e: React.KeyboardEvent) => {
    if (e.key === "Enter" || e.key === " ") {
